  ...
  render(){
     return <div className="container">
      ...
      <div className="row mt-5 mb-5">
        <div className="col-5 ">
          <input style={{'borderRadius':'10px','boxShadow':'1px 1px 1px' , width: '60%'}} onBlur={this.validateRoll} ref={(roll)=>this.roll = roll} type="text" className="form-control" placeholder='Enter Roll Number'/>
          {this.state.rollStatus && <small className='text-danger'>Roll number is already taken</small>}
        </div>
        <div className="col-5">
          <input style={{'borderRadius':'10px','boxShadow':'1px 1px 1px' , width: '60%'}} ref={(name)=>this.name = name} type="text" className="form-control" placeholder='Enter Name'/>
        </div>
      </div>
      <div className="row mt-5 mb-2">
        <div className="col-5">
          <input style={{'borderRadius':'10px','boxShadow':'1px 1px 1px' , width: '60%'}} ref={(mobile)=>this.mobile = mobile} type="text" className="form-control" placeholder='Enter Mobile Number'/>
        </div>
        <div className="col-5">
          <select style={{'borderRadius':'10px','boxShadow':'1px 1px 1px' , width: '60%'}} ref={(branch)=>this.branch = branch} className="form-control">
      ...
      <div className="row mt-5 mb-2">
        <div className="col-2">
          <button onClick={this.addStudent} style={{width:'80%'}} className="btn btn-primary">Add</button>
        </div>
      ...
      <hr/>
      <table style={{boxShadow:"1px 2px 5px black"}} className="table">
         <thead style={{boxShadow:"1px 1px 1px black"}}>
            <tr>
              <th>S.no</th>
              <th>Roll</th>
              <th>Name</th>
